...
.DESCRIPTION
    ...
    and build-time crates are not in the binary, so a notice for them would
    claim something untrue about what a user has been given; `deny.toml` and CI
    still hold those to the licence allow-list, which is a different question.

    Clipped's own crates are not listed either: they are covered by LICENSE at
    the root of the payload, which is the same file.
...
function Get-ShippedCrates {
    ...
    foreach ($manifest in $Manifests) {
        ...
        foreach ($member in $metadata.workspace_members) { [void] $members.Add($member) }
...
"@
    ...
    Copy-Item -LiteralPath $licence -Destination (Join-Path $Destination 'LICENSE.txt')
    Copy-Item -LiteralPath $notices -Destination (Join-Path $Destination 'THIRD-PARTY-NOTICES.md')

    $ffmpegDirectory = Join-Path $Destination 'ffmpeg'
    ...
    Write-Payload -Path (Join-Path $ffmpegDirectory 'NOTICE.md') -Lines (@(
            ...
            'available to whoever received them. It is published with the Clipped release',
            'that carries these files; `scripts/fetch-ffmpeg-source.ps1` in the Clipped',
            'repository assembles it, and writes a manifest naming the FFmpeg commit and the',
            'build recipe these binaries were produced from.',
            '',
    ...
    $rustNoticeFile = Join-Path $Destination 'THIRD-PARTY-NOTICES-RUST.md'
    $crateCount = 0
    ...
    if ($SkipRustDependencies) {
        ...
        $lines = @(
            ...
            '',
            'These are the third-party Rust crates compiled into the binaries this',
            'application is made of, with the licence notice each one publishes.',
            '',
            ...
            '`dev-dependencies` and `build-dependencies` are excluded: they are not in the',
            'binaries, so a notice for them would say something untrue about what you have',
            'been given. Clipped''s own crates are excluded too - they are covered by',
    ...
    Write-Payload -Path (Join-Path $Destination 'README.md') -Lines @(
        ...
        '',
        "Generated by ``scripts/collect-notices.ps1`` on $((Get-Date).ToUniversalTime().ToString('yyyy-MM-dd')). Do not edit by hand: run it again."
    )
