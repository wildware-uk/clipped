...
.DESCRIPTION
    ...
    directory, except that -PersistEnvironment writes FFMPEG_DIR to the user
    environment and, under GitHub Actions, FFMPEG_DIR is appended to the file
    named by GITHUB_ENV so later steps inherit it.
...
.PARAMETER PersistEnvironment
    Also write FFMPEG_DIR to the current user's environment, so new shells find
    it without re-running this script. Off by default because writing to a
    contributor's environment behind their back is rude.
...
function Test-InstalledPin {
    ...
    try {
        ...
        Write-Step "Ignoring unreadable pin record at $pinFile."
        return $false
    }
...
function Publish-FfmpegDirectory {
    <#
    .SYNOPSIS
        Reports FFMPEG_DIR and, where asked to, makes it stick.
    .DESCRIPTION
        FFMPEG_DIR is the one variable the build needs: ffmpeg-sys-the-third
        reads it and derives the include and lib directories from it. The
        runtime DLLs are handled separately - crates/muxer's build script copies
        them next to the binaries it builds - so PATH is deliberately left
        alone.
    #>
    $env:FFMPEG_DIR = $buildRoot

    if ($PersistEnvironment) {
        [Environment]::SetEnvironmentVariable('FFMPEG_DIR', $buildRoot, 'User')
        Write-Step 'Wrote FFMPEG_DIR to the user environment; new shells will pick it up.'
    }

    if ($env:GITHUB_ENV) {
        Add-Content -LiteralPath $env:GITHUB_ENV -Value "FFMPEG_DIR=$buildRoot"
        Write-Step 'Exported FFMPEG_DIR through GITHUB_ENV for later workflow steps.'
    }

    Write-Host ''
    Write-Host 'FFmpeg is ready. Set this before building, if it is not set already:'
    Write-Host ''
    Write-Host "    `$env:FFMPEG_DIR = '$buildRoot'"
    Write-Host ''
    if (-not $PersistEnvironment) {
        Write-Host 'Re-run with -PersistEnvironment to write it to your user environment once and stop thinking about it.'
    }
}
