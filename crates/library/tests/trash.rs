...
#[test]
fn stowing_an_unindexed_fragment_moves_it_but_leaves_no_row_behind() {
    // `clipped-recorder recover --discard` (issue #451) hands the trash a file
    // that has never been through indexing at all — an interrupted recorder's
    // leftovers, not a row from `recordings`. The physical move has to work
    // exactly as it does for a tracked item; what has to be proved is the
    // opposite of `deleting_a_recording_moves_the_file_rather_than_unlinking_it`
    // above -- that nothing here quietly gives the file a row, because a row
    // this function did not ask for is exactly the "tail wagging the dog"
    // shortcut it was written to avoid.
    let library = Library::new("untracked");
    let database = library.index();
    let orphan = library.file("clipped-cs2-20260811-150000.mkv");
    fs::write(&orphan, footage(0, 4_096)).expect("a recording can be written");

    let before_recordings = count(&database, "SELECT COUNT(*) FROM recordings");
    let before_sessions = count(&database, "SELECT COUNT(*) FROM sessions");

    let stowed = library
        .trash
        .stow_untracked(&orphan, deleted_at())
        .expect("the file can be moved");

    let moved_to = stowed.path.expect("there was a file to move");
    assert!(!orphan.exists(), "the file was left where it was");
    assert!(
        moved_to.starts_with(library.trash.directory()),
        "the file went somewhere other than the trash: {}",
        moved_to.display()
    );
    assert_eq!(
        fs::read(&moved_to).expect("the moved file can be read"),
        footage(0, 4_096),
        "the footage changed on the way into the trash"
    );
    ...
    assert_eq!(
        count(&database, "SELECT COUNT(*) FROM recordings"),
        before_recordings,
        "an untracked stow must not insert a recordings row"
    );
    assert_eq!(
        count(&database, "SELECT COUNT(*) FROM sessions"),
        before_sessions,
        "an untracked stow must not insert a sessions row either"
    );
    assert!(
        library
            .trash
            .list(&database)
            .expect("the trash can still be listed")
            .is_empty(),
        "a file with no row must not appear in the trash listing"
    );
}

#[test]
fn stowing_a_fragment_that_has_already_gone_reports_nothing_to_move() {
    // The same "already gone" answer `send` gives a tracked item with no file
    // behind it -- an interrupted recorder that was killed before the encoder
    // wrote a first packet leaves exactly this (issue #283).
    let library = Library::new("untracked-missing");
    let never_existed = library.file("clipped-cs2-20260811-150000.mkv");

    let stowed = library
        .trash
        .stow_untracked(&never_existed, deleted_at())
        .expect("a missing file is not an error");

    assert_eq!(stowed.path, None);
    assert!(
        !library.trash.directory().exists(),
        "an entry directory was made for a file that was never there"
    );
}
