...
use super::{classify, picture_kind, settings, sys, NvencEncoder};
...
fn a_panic_while_nvenc_holds_the_texture_still_gives_it_back() {
    ...
    //
    // So this runs the unwind the comment has to survive: a panic raised while
    // NVENC holds a registration on a texture this encoder does not own.
    //
    // What it observes is the guard: `registrations` counts live
    // `Registration`s, each of which unmaps and unregisters in its `Drop`, so a
    // count of zero after the unwind is the release having happened rather than
    // a driver-side query. Delete the `Drop` impl and this fails on the count;
    // the encode afterwards is there because a count can only say the guard
    // ran, not that what it did left a usable session.
    let Some(gpu) = TestGpu::open() else {
        return;
    };
    ...
    let unwound = std::panic::catch_unwind(AssertUnwindSafe(|| {
        ...
        let registration = session
            .register_and_map(&source_frame(&texture, 0), format, TEST_SIZE)
            .expect("the texture registers and maps");
        assert!(
            !registration.registered.is_null() && !registration.mapped.is_null(),
            "NVENC reported success without handing back a registration, so the panic below \
             would prove nothing"
        );
        assert_eq!(
            session.registrations.get(),
            1,
            "the registration should be live here, or this test unwinds through nothing"
        );

        panic!("a deliberate panic while NVENC holds the caller's texture");
    }));
    assert!(unwound.is_err(), "the payload was supposed to panic");
