...
//! games, and generating a waveform or a thumbnail for a library of
//! recordings is exactly the background work that must not take anything a
//! game needs. Two things are asked of Windows, and both matter.
//!
//! **`THREAD_PRIORITY_LOWEST`** drops the thread three levels below normal,
//! so the scheduler runs it only when nothing else in its priority class
//! wants the processor.
//!
//! **`THREAD_MODE_BACKGROUND_BEGIN`** does more than that: it drops the
//! thread to the lowest scheduling priority *and* puts its disk reads at
//! background I/O priority. That second half is the important one here.
//! Reading a finished recording end to end means reading the whole file, and
//! a recording in progress is writing to the same disk; a low-priority thread
//! issuing normal-priority reads would still take disk bandwidth from the
//! recorder. Background I/O priority is what Windows gives its own indexer
//! for the same reason.
//!
//! Both are per-thread and reversible, and this crate applies them to a
//! thread `crate::worker` created and owns, never to a caller's.
//!
//! # When it does not work
//!
//! `SetThreadPriority` can fail. The worker reports what actually happened
//! through [`crate::WorkerPriority`] rather than assuming, because "we asked
//! for background priority" and "the thread is running at background
//! priority" are different statements and only the second one is worth
//! anything.
...
use crate::worker::WorkerPriority;
...
pub(crate) fn enter() -> WorkerPriority {
    ...
    // thread. It needs no closing and is valid wherever it is used on this
    // thread.
    ...
    // Windows reports the background value instead, which is lower still but
    // is not `THREAD_PRIORITY_LOWEST`.
    //
    ...
    // background mode, which is why the result is reported rather than
    // checked.
...
/// — which this crate does not do today, but a future host might — is
/// neither left in background I/O mode nor left three levels below normal.
/// Ending background mode does not restore the scheduling priority:
/// `THREAD_MODE_BACKGROUND_END` puts back whatever was set before background
/// mode began, which here was `THREAD_PRIORITY_LOWEST`, so the second call is
/// what actually makes the thread ordinary again.
pub(crate) fn leave() {
    // SAFETY: as in `enter`.
    let thread = unsafe { GetCurrentThread() };
...
mod tests {
    ...
    fn leaving_the_background_puts_the_thread_back_to_an_ordinary_priority() {
        // On a thread of its own: this changes the priority of whichever
        // thread it runs on, and the test harness's threads are not this
        // test's to alter.
        std::thread::spawn(|| {
            let entered = enter();
