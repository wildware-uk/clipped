...
> **The binaries are packaged; the paperwork is not.**
> `npm run build:app` now produces an installer carrying the recorder and the
> seven FFmpeg DLLs beside the window
> ([#226](https://github.com/wildware-uk/clipped/issues/226),
> [docs/packaging.md](packaging.md)) — which is the half of the obligations
> below whose subject is *the libraries*: they are conveyed unmodified, as
> ordinary files, replaceable in place. It carries **no licence texts and no
> notices**: `bundle.resources` names only the payload
> `scripts/stage-installer-payload.ps1` stages, and the notices payload is not
> in it. So every obligation below whose subject is "an installed build
> *contains* a text" is still **stated but not met**, and an installer built
> today is not one that may be distributed. What can exist now does: the notices
> payload is generated by a script, the corresponding source is fetched by a
> script, and the relinking permission has been tested against real binaries.
> Each section says where it stands.
>
> Nothing can distribute one by accident in the meantime.
...
> [docs/releasing.md](releasing.md#the-licence-gate). When
> [#123](https://github.com/wildware-uk/clipped/issues/123) puts the payload in
> the bundle, that gate starts passing on its own.
