...
all of `ipc_protocol.rs` except its four:
...
and `a_recording_the_watcher_started_names_the_game_over_the_protocol` — all of
which need a GPU, an encoder and a desktop session, because each is about a
recording and the last of them also waits for this machine's process detection
to notice a launch. Telling `watching` from `idle` needs none of that and runs in
...
The four `#[ignore]`d tests in `ipc_protocol.rs` record a real window, which
