...
all of `ipc_protocol.rs` except its two:
`a_recording_driven_entirely_over_the_protocol_produces_a_playable_file` and
`a_watching_recorder_moves_through_all_three_states_over_the_protocol`, which
need a GPU, an encoder and a desktop session because a recording is one of the
three states. Telling `watching` from `idle` needs none of that and runs in CI,
which is where that regression has to be caught.
...
The two `#[ignore]`d tests in `ipc_protocol.rs` record a real window, which comes
from a test application rather than an example:
