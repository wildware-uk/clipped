...
| `recorded_frames.rs`     | The same three questions asked of a **finished recording** rather than of a capture ([#183](https://github.com/wildware-uk/clipped/issues/183)): the subject is recorded, every picture is extracted as BGRA with the pinned FFmpeg, and the counter is read back out of each. Duplication and disorder are held to zero — neither has an honest cause — while missing frames are checked against what `RecordingReport` itself admits dropping. The counting is `clipped_video_pattern::sequence::CounterRun`, shared with the row above so the two cannot disagree |
...
The per-game settings on their own, which is issue #61's first acceptance
criterion and takes about two and a half minutes — it records two subjects, one
after the other, because this process records one thing at a time whoever asked
for it:

```text
cargo build -p clipped-video-pattern
cargo build -p clipped-recorder
$env:CLIPPED_REQUIRE_MEDIA = "1"
cargo test -p clipped-recorder --test automatic_sessions \
  each_game_records_at_the_settings_its_own_entry_asks_for -- --ignored --nocapture
```

It prints each file's size, codec, decoded picture count and observed frame rate
beside the command line and the global layer it was recorded against, whether it
passes or fails. It needs a display large enough for a 2560x1440 borderless
window; the two subjects open at that and at 1920x1080, and each opens on a
display that is not the primary one where there is one. A workspace is kept under
`%TEMP%\clipped-watch-per-game-…` **only when the run failed**, with the path
printed.

The quality presets on their own, which is issue #62's acceptance criteria and
takes about twenty seconds — it records the same window four times, once per
preset:

```text
cargo build -p clipped-video-pattern
$env:CLIPPED_REQUIRE_MEDIA = "1"
cargo test -p clipped-recorder --test record_end_to_end   each_quality_preset_opens_the_encoder_session_it_asks_for_and_only_one_changes_the_codec   -- --ignored --nocapture
```

It prints each file's codec, size and frame count, whether it passes or fails.
Two of its three readings come off the line the encoder backend logs rather than
off the file, and the test says why: a recording of the test pattern is far too
compressible for the rate control to bind, so the bitrate a preset chose is not
in the file at any resolution. What *is* in the file is the codec — Performance
asks every encoder for H.264 and the rest take the most efficient one it was
measured to support — and that is the reading that makes the test causal, because
all four invocations pass the same `--codec auto` against the same window.

`clipped-recorder capabilities` is the other half of the same evidence: it prints
what all four presets resolve to on every encoder it found, so a machine with two
GPU classes in it answers issue #62's first criterion for itself.
