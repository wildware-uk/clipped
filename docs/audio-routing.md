...
**Status: one stream exists, and it is not a track model yet.**
...
through, as a continuous, timestamped stream of `f32` samples. That is the
foundation the track model is built on, and it is all there is. Routing — the
part this document is named after — is milestone M2, and the sections at the
end that describe it are still unwritten because describing behaviour before it
is built produces a page that is wrong from the day it is committed (AGENTS.md
section 7).

So this document describes one capture: what it does, the two problems it
exists to solve, what it converts, what happens when the user changes their
audio device mid-recording, and how to check any of it for yourself. The
...
`IMMNotificationClient` and watches for the console render default moving, for
the captured device leaving the active state, and for it being removed.
...
threads and can give them the priority and lifetime it wants. `SystemAudioCapture`
is `Send` so it can be built anywhere and moved onto that thread, and is not
`Sync`.
...
- The console render role is what is recorded. The communications role — which
  a headset may hold while speakers hold the console role — belongs to
  microphone capture and voice chat
