 ...
 *
 * The component does not drive the window itself: `@clipped/ui` renders in a
 * test runner as readily as it does in a Tauri window, and it can only do that
 * if nothing in it imports a Tauri API. The application supplies these.
 */
 ...
 *
 * Clipped draws one rather than using the system's because the design system
 * specifies it: a dark bar carrying the brand mark, with the window controls
 * flush right. The trade is that Windows 11's Snap Layouts flyout - which
 * appears when the pointer rests on a *system* maximise button - is not
 ...
 *
 * `data-tauri-drag-region` is what makes the bar draggable, and what makes a
 * double-click on it maximise the window the way a system title bar does. It
 * is set on the brand block rather than on the whole bar so that the region
 * can never swallow a click meant for a button.
 */
...
/* The three window-control glyphs, drawn rather than imported. The design
   ...
   that. `currentColor` is what lets the close button turn its glyph over to the
   ground colour on hover without a second rule.

   Each is `aria-hidden`: the button around it carries the name. */

const GLYPH: { width: number; height: number; viewBox: string; 'aria-hidden': true } = {
  width: 10,
  height: 10,
  viewBox: '0 0 10 10',
  'aria-hidden': true,
...
function RestoreGlyph(): JSX.Element {
  return (
    <svg {...GLYPH}>
      <path d="M2.5 2.5h7v7" fill="none" stroke="currentColor" strokeWidth="1" />
      <rect
        x="0.5"
        y="4.5"
        width="7"
