...
describe('Sidebar', () => {
  ...
  it('reaches every screen with the keyboard alone, and activates with Enter', async () => {
    ...
    // Tab from the top of the document to the third entry, then press Enter.
    // This is the whole of AGENTS.md section 46's "keyboard navigation" claim
    // for the sidebar: no pointer is involved at any point.
    await user.tab();
    await user.tab();
    await user.tab();
  ...
  it('renders a footer only when it is given one', () => {
    const { rerender, container } = render(<Sidebar currentScreenId="home" onNavigate={vi.fn()} />);
