...
impl RecordingState {
    ...
    pub(crate) fn adopt(
        ...
        *current = Some(Running {
            ...
            // recorder's own words. Whether an automatic recording should keep
            // one is a decision about memory — roughly 140 MiB a minute — and is
...
mod tests {
    ...
    fn state_over(directory: &Path, catalogue: Catalogue) -> Arc<RecordingState> {
        Arc::new(RecordingState::new(
            ...
            // Nowhere: a test must not read or write the settings of whoever is
            // running it (AGENTS.md section 25).
            Arc::new(crate::settings::SettingsFile::nowhere()),
            catalogue,
    ...
    fn a_replay_asked_for_of_an_automatic_recording_is_refused_rather_than_taken() {
        // An automatic recording keeps no buffer — that is issue #427 — so
        // `save_replay` has to refuse it in the same words it refuses a
        // window-started recording without one, and must not reach for a
